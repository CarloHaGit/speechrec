...
function setup() {
  ...
  infoButton.position(420, 30);
  infoButton.mousePressed(showInfo);
  
  resetButton = createButton("Reset Sliders");
  resetButton.position(32, 310);
...
function drawType(charString, charX, charY, parSize, spacing, charStroke) {
 ...
 // if(!infoMode){
   ...
   /* if (currentX + charWidth + spacing > windowWidth - 150)       {
    ...
    if (currentX + charWidth + spacing > windowWidth - 150) {
      ...
      if (currentY + 298.5 < 610) {
        ...
        console.log(currentY + 262 + 30);
        console.log(currentY + 262 + 30 < 910);
        //currentY = startingY;
        //currentX = startingX;
        //mode2 = true;
  ...
  if(infoMode){
    text("Welcome to my project for the Word as Image Class. Throughout the class I explored connecting type and sound. Feel free to just talk to this website and tweak the specimen by experimenting with the sliders. If you want to reset them, just click here:", 35, 100, 450,1000);
  }
  pop();
}
